...
const Sidebar = () => {
  ...
  return (
    <div className='fixed left-0 top-0 min-h-screen w-64 z-10 p-10 bg-[#F4F5F5] shadow-md'>
      ...
      <div className = "mt-7 flex gap-3 items-center align-middle mb-10">
      ...
      {Items.map((item, index) => (
        <div key={index} className='text-neutral-500 p-2 flex gap-4 mt-3 text-sm items-center cursor-pointer hover:bg-neutral-200 w-full rounded'>
          {item.icon}
          {item.name}
        </div>
      ))}
